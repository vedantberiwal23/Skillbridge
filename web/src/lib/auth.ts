 ...
 *   2. DEV_PREVIEW_ROLE (or a `dev_role` cookie) must be set explicitly — it is
 *      off even in dev unless someone opts in, and it is a server-side var, so
 *      it is never inlined into client JS.
 *
...
async function devPreviewSession(): Promise<SessionUser | null> {
  ...
  try {
    devCookie = (await cookies()).get('dev_role')?.value;
  } catch {
    // Outside Next.js request store
  }
  ...
  return {
    ...
    role,
    deptId: process.env.DEV_PREVIEW_DEPT_ID ?? null,
  };
}
 ...
 *
 * The dev preview is consulted here and in `gatePage` only, never inside
 * `requireSession` — route handlers are the enforcement boundary and must not
 * be satisfiable by an env var.
 */
export async function getSession(): Promise<SessionUser | null> {
  const session = await requireSession().catch(() => null);
  return session ?? (await devPreviewSession());
}
 ...
 *
 * ROLES is ordered least- to most-privileged, so an admin may open a manager
 * page — the same rule `requireSession` applies.
 */
export async function gatePage(requiredRole: Role): Promise<SessionUser> {
  const session = await getSession();
  if (!session) redirect('/login');
  if (rank(session.role) < rank(requiredRole)) redirect(HOME_FOR_ROLE[session.role]);
  return session;
}
